...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    ...
    const REDIRECT_URI = makeRedirectUri({ useProxy: true }); //pega automaticamente a URL da twitch auth.expo.io/ etc
    ...
    if(response.type === 'success' && response.params.error !== 'access_denied'){
          ...
          const userResponse = await api.get('/users'); //api = componente baseURL = "https://..../users"
  ...
  async function signOut() {
    try {
      ...
      setUser({} as User);
      setUserToken('');
      //Remover o access_token do cabeçalho das requisições. Para implementar isso, utilize:
